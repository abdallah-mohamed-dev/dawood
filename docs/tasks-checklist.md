...
> ## 📦 أرشيف مغلق — ممنوع الكتابة هنا
> الملف دا بيوثّق تنفيذ الـMVP (Tasks 0–11) **اللي خلص بالكامل**. محدش يضيف أو يعدّل فيه.
> **الشغل الجديد كله مكانه [`../specs/`](../specs/README.md)** — ملف لكل تاسك كبير.

> مرجع: `docs/tasks.md`. كل تاسك هنا سطر أو اتنين.
...
- [x] رابط الشركاء في شريط التنقل (بعد Task 9)
- [x] توحيد رسائل النجاح/الخطأ (flash) بشكل أساسي
- [x] توحيد شكل عرض أخطاء الـvalidation في كل الصفحات (نفس نمط `@error` + `text-danger` في كل مكان، اتأكد منه بالتدقيق)
- [x] Blade components مشتركة (جدول/نموذج/زر حذف) — `<x-data-table>`, `<x-field>`, `<x-delete-button>`
- [x] مراجعة أن كل المبالغ تُعرض حصرًا عبر <x-money>
- [x] تنسيق موحد لعرض كل التواريخ
- [x] Empty states واضحة لكل جدول
- [x] تدقيق Design Tokens (بحث hex/font-family خارج app.css)
- [x] تصحيح أي مخالفات يكشفها التدقيق (صفر مخالفات — التدقيق كان نظيفًا بالفعل)

## Task 11 — اختبار الدورة الكاملة (Integration Test)

- [x] اختبار Pest شامل: رصيد افتتاحي → شراء → مخزون
- [x] نفس الاختبار: غرفة → صرف خامات → تكلفة
- [x] نفس الاختبار: دفعة عميل → مصروف → إكمال الغرفة
- [x] نفس الاختبار: حساب الربح → نصيب شريك → سحب
- [x] التحقق أن كل أرقام الجدول المرجعي مطابقة (خزنة 14,500 / ربح 27,460)
- [x] التحقق: لا حركة خزنة بلا مصدر (عدا الرصيد الافتتاحي)
- [x] التحقق: لا حركة مالية محسوبة مرتين (مقارنة يدوية)

---

**الإجمالي:** Tasks 0–11 مكتملة بالكامل (اختبارات: 223/223 ناجحة، Pint نظيف). البند الوحيد غير المعلَّم في كل الملف هو "عرض الشكل النهائي على المستخدم والتوقف لأخذ الملاحظات" (Task 8.7) — بند تفاعلي يحتاج المستخدم نفسه، مش شيء يُنفَّذ بالكود.
