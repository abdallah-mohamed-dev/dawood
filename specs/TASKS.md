...
- [ ] **6.1** `MaterialController::index()` — `paginate(50)` + `->withQueryString()` (عشان البحث `q` مايضيعش في الصفحة التانية)
- [ ] **6.2** **⚠️ مهم:** `stockByMaterialIds()` دلوقتي بتجيب مخزون **كل** المواد — تتقيّد بمواد الصفحة الحالية بس
- [ ] **6.3** `resources/views/inventory/materials/index.blade.php` — `{{ $materials->links() }}`
- [ ] **6.4** `ExpenseCategoryController::index()` — `paginate(25)` + links
- [ ] **6.5** اختبارات: الصفحة التانية شغالة • البحث بيفضل شغال مع الباجينيشن • مخزون الصفحة الحالية بس بيتجاب
- [ ] **6.6** `php artisan test --compact` + `pint` + `npm run build`
- [ ] **6.7** إدخال في `AGENT_LOG.md`
...
- [ ] **7.1** `resources/views/components/quick-add.blade.php` — كومبوننت واحد للفورم الأفقي (**تجريد مبرر: 6 استخدامات فعلية**)
- [ ] **7.2** المخزون — فورم: اسم المادة · وحدة القياس · زر. حذف `create` (كونترولر + راوت + فيو)
- [ ] **7.3** العملاء — فورم: الاسم · الهاتف · العنوان · زر. حذف `create`
- [ ] **7.4** بنود المصروفات — فورم: اسم البند · زر. حذف `create`
- [ ] **7.5** المشتريات — فورم: المادة · الكمية · سعر الوحدة · التاريخ · طريقة الدفع · زر. حذف `create`
- [ ] **7.6** المصروفات — فورم: البند · المبلغ · التاريخ · الوصف · طريقة الدفع · زر. حذف `create`
- [ ] **7.7** **صفحة العميل** — فورم إضافة غرفة: نوع الغرفة · سعر البيع · زر (العميل معروف من الصفحة). حذف `rooms.create`، و`RoomController::store` تاخد الـ`customer_id` من المسار
- [ ] **7.8** `routes/web.php` — تنضيف كل راوتات `create` الملغاة
- [ ] **7.9** تحديث كل الاختبارات اللي بتضرب على `*.create` — تتحوّل لاختبار إن الفورم ظاهر في `index`
- [ ] **7.10** اختبارات جديدة: خطأ validation بيرجّع للصفحة نفسها بالقيم والأخطاء • مايسربش لفورم تاني في نفس الصفحة
- [ ] **7.11** `php artisan test --compact` + `pint` + `npm run build`
- [ ] **7.12** `USER-GUIDE.md` — الإضافة بقت من نفس الصفحة
- [ ] **7.13** إدخال في `AGENT_LOG.md`
