...
| 8 | فلاتر وبحث المشتريات | ✅ **مكتمل** (322/322 اختبار) |
| 9 | فواصل الشهور في المصروفات | ✅ **مكتمل** (327/327 اختبار) |
...
# الدفعة الثالثة — النسخ الاحتياطي والتصدير (تاسكات 10 → 14)

**قرارات المستخدم المعتمدة (لكل الدفعة دي):**

- **صيغة التصدير: CSV بسيط** — `fputcsv` مباشر، من غير أي مكتبة خارجية (`composer require` ممنوع هنا). بيتفتح في Excel عادي زي ملف Excel.
- **صفحة واحدة تجمع كل حاجة:** `GET /backup` (كنترولر `BackupController`، رابط جديد في الـSidebar اسمه "النسخ الاحتياطي"). كل الأزرار (نسخة قاعدة البيانات + CSV شامل + تصدير كل قسم) في نفس الصفحة — **مفيش صفحات أو أزرار تصدير جوه صفحات المخزون/العملاء/... الموجودة**، عشان كل تاسك يفضل مستقل تمامًا عن الصفحات الحالية ومايعملش تعارض تعديلات معاها.
- **نسخة قاعدة البيانات الكاملة:** القاعدة SQLite (`database/database.sqlite`) — الباكب هو **نسخ الملف نفسه** وإرجاعه بـ`response()->download()` باسم فيه التاريخ (`dawood-backup-2026-09-01.sqlite`). ممنوع أي `mysqldump`-style logic، القاعدة SQLite مش MySQL.
- **CSV الشامل:** ملف `.zip` فيه ملف CSV منفصل لكل جدول رئيسي (customers, rooms, materials, inventory_batches, purchases/inventory_movements, room_materials, room_costs, customer_payments, expenses, cashbox_transactions, partners, partner_withdrawals) — يتبني بـ`ZipArchive` في storage مؤقت ويتحذف بعد التنزيل.
- **الغرف في التصدير:** أي تصدير فيه غرف (شامل أو منفصل) لازم عمودين إضافيين: اسم العميل ورقم تليفونه (join مش عمود مخزّن).
- **"الحسابات":** المقصود بيها دفعات العملاء + المصروفات + الخزنة + الشركاء وسحوباتهم مجمّعين في تصدير واحد بأربع تبويبات/ملفات، مش كيان واحد جديد.
- **صلاحيات:** الصفحة والراوتس جوه `middleware('auth')` زي كل حاجة تانية، مفيش تعقيد إضافي (مستخدم واحد بس في النظام).
- **الباكب اليدوي فقط:** لا جدولة تلقائية (cron) ولا حفظ نسخ قديمة على السيرفر — كل ضغطة زرار بتولّد وتنزّل الملف على طول ومفيش تخزين دائم لنسخ سابقة.

| # | التاسك | الحالة |
|---|---|---|
| 10 | صفحة الباكب الأساسية + تحميل نسخة كاملة من قاعدة البيانات (sqlite) | ✅ **مكتمل** (331/331 اختبار) |
| 11 | تصدير CSV شامل لكل جداول النظام (zip) في نفس صفحة الباكب | ✅ **مكتمل** (333/333 اختبار) |
| 12 | تصدير CSV منفصل: العملاء + الغرف (باسم وتليفون العميل) | ⬜ لسه |
| 13 | تصدير CSV منفصل: المخزون (المواد) + المشتريات | ⬜ لسه |
| 14 | تصدير CSV منفصل: الحسابات (دفعات العملاء + المصروفات + الخزنة + الشركاء وسحوباتهم) | ⬜ لسه |

**الترتيب إلزامي:** 10 → 11 → 12 → 13 → 14.
10 هو الأهم والأبسط (نسخ ملف واحد، بدون أي منطق تصدير) — لازم يخلص الأول عشان لو التوكنز خلصت يفضل في الأقل باكب شغال للقاعدة كلها. 11 بيبني على نفس صفحة/كنترولر 10. 12/13/14 مستقلين عن بعض تمامًا بعد ما 10 يخلّص (كل واحد بيضيف زرار وميثود جديدة بس)، فلو اتوقفوا في أي نقطة النظام يفضل شغال زي ما هو من غير أي كسر.

**كل تاسك من دول لازم يتعمله ملف `specs/0NN-...md` من `_TEMPLATE.md` قبل التنفيذ** (فيه خطوة migrations→...→tests زي باقي التاسكات) — الدفعة دي بس بتوثّق القرارات المعمارية والترتيب مقدمًا عشان أي جلسة/موديل جديد يقدر يكمل من غير ما يعيد النقاش.
...
- [x] **8.1** `PurchaseController::index()` — يقبل: `q` (اسم المادة) · `from` / `to` (تاريخ) · `status`، كلها `when()` وكلها `->withQueryString()`
- [x] **8.2** تعريف الحالة: `available` = `remaining_quantity = quantity` • `partial` = بين الاتنين • `depleted` = `remaining_quantity = 0`
- [x] **8.3** `InventoryService::purchasesSummary()` — تستخدم **نفس** صيغة `cost()` مش نسخة تانية
- [x] **8.4** شريط فلاتر فوق الجدول (فورم `GET` واحد) + رابط "إلغاء الفلاتر" + زرين اختصار: "الشهر ده" / "الشهر اللي فات"
- [x] **8.5** شريط ملخص: "N عملية شراء — إجمالي X ج.م"
- [x] **8.6** رسالة `empty` مخصصة لما الفلترة مترجعش حاجة
- [x] **8.7** اختبارات: كل فلتر لوحده • مع بعض • بيفضلوا مع الباجينيشن • الملخص بيتغير مع الفلترة • الإجمالي مطابق لمجموع `cost()`
- [x] **8.8** `php artisan test --compact` + `pint` + `npm run build`
- [x] **8.9** `USER-GUIDE.md` + إدخال في `AGENT_LOG.md`
...
- [x] **9.1** `ExpenseController::index()` — استعلام تجميعي مستقل: إجمالي كل شهر → مصفوفة مفتاحها `Y-m`
- [x] **9.2** `resources/views/expenses/index.blade.php` — في اللوب: مقارنة `occurred_at->format('Y-m')` بالصف اللي قبله، ولو اختلف يطبع `<tr>` فاصل بـ`colspan` كامل
- [x] **9.3** الفاصل: اسم الشهر بالعربي + السنة + "إجمالي الشهر: X ج.م" (**ألوان من الـtokens بس، ممنوع hex**)
- [x] **9.4** أسماء الشهور العربية من `lang/ar/` مش hardcoded في الـBlade
- [x] **9.5** اختبارات: الفاصل بيظهر عند تغيير الشهر • مابيظهرش بين صفين في نفس الشهر • إجمالي الشهر صح حتى لما الشهر مقسوم على صفحتين
- [x] **9.6** `php artisan test --compact` + `pint` + `npm run build`
- [x] **9.7** إدخال في `AGENT_LOG.md` (**مفيش تحديث لـ`USER-GUIDE.md`** — تغيير عرض بحت بدون أثر على المنطق)
