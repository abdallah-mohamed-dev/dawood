...
<div class="flex h-full flex-col rounded-xl border border-border bg-surface p-4 shadow-sm">
    ...
    <form method="POST" action="{{ route('rooms.costs.store', $room) }}" class="mb-4 grid grid-cols-2 gap-3">
        ...
        <x-payment-method-select :bag="$bag" :id="$bag . '_payment_method'" width="w-full" />

        <button type="submit" class="self-end rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white shadow-sm transition-all hover:bg-primary-dark hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary/40 focus:ring-offset-2">{{ __('Add') }}</button>
    </form>
