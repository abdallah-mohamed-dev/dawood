<x-app-layout title="تعديل عميل">
    <h1 class="mb-6 text-2xl font-bold tracking-tight text-gray-900">تعديل عميل</h1>
    ...
    <form method="POST" action="{{ route('customers.update', $customer) }}" class="max-w-md space-y-4">
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('customers.show', $customer) }}" class="rounded-md border border-border px-4 py-2 text-sm text-gray-700 hover:bg-bg">
                {{ __('Cancel') }}
            </a>
        </div>
    </form>
